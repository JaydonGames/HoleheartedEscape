...
Render::Rect Tile::get_coords() { return this->get_data().coords; }

unsigned int Tile::get_flags() { return this->render_flags; }
...
Map::Map(const char *map_string) {
  ...
  for (tinyxml2::XMLElement *layer_elem = map->FirstChildElement("layer");
    ...
    if (std::string_view{encoding} == "csv")
      layer.tiles = parse_csv_grid(layer.width, layer.height, data->GetText(),
                                   tileset_gids);
...
void Map::register_tileset(const char *filename, const char *tileset_string) {
  ...
  int tile_width, tile_height, tile_count, column_count, image_width,
      image_height;
  ...
  for (size_t i = 0; i < tile_count; ++i) {
    ...
    tiles.emplace_back(
        Render::Rect{x * tile_width, y * tile_height, tile_width, tile_height},
        Map::get_texture(image_file));
  }

  tileset_names[filename] =
      &Map::tilesets.emplace_back(std::move(tiles)) - Map::tilesets.data();
}

void Map::register_texture(const char *name, OpenGL::Texture &&texture) {
  Map::texture_names[std::string{name}] =
      &Map::textures.emplace_back(std::move(texture)) - Map::textures.data();
}

OpenGL::Texture &Map::get_texture(size_t id) { return Map::textures[id]; }
